...
def serchoperator(name):
    ...
    promotion_resources = collect_data(promotion_tds, 1)

    skills_tds = skils.find_all('td')
    skils_resourses = collect_data(skills_tds, 2)


    modeules_resourses = {}
    for enum, item in enumerate(modeules, start=1):
        module_tds = item.find_all('td')
        modeules_resourses[enum] = collect_data(module_tds, 1)
